...
public:
	BCGL_Sprite();
	BCGL_Sprite(COORD size);
	BCGL_Sprite(CHAR_INFO* string, COORD size);
	BCGL_Sprite(char* string, WORD attributes, COORD size);

	void freeMemory();									// Free memory allocated within this object
	bool serialize(const char* path);					// Serialize this sprite to a file
	static BCGL_Sprite* deserialize(const char* path);	// Create a sprite object from a serialized sprite file
